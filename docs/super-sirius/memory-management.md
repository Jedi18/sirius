...
Before history is available, the task takes the maximum of its operators' `no_history_peak_memory_estimate(input_stats)` values. Operators without an override estimate `2 × input bytes`. An explicit zero is authoritative: a single-partition PARTITION or a CONCAT with at most one input batch contributes no additional GPU peak. Allocating PARTITION tasks estimate `2 × input bytes` (also when the partition count is not yet known), and multi-batch CONCAT estimates `input bytes`. Only an absent or empty operator chain uses the task-level `2 × input bytes` fallback. Input materialization is still added to the operator peak, and the task's OOM retry reservation floor still applies even when that peak is zero.
