...
3. **Vendor** `sqlsmith.duckdb_extension` for v1.5.5 (linux_amd64/arm64) next to the TPC-DS fixture
   ...
   `duckdb/.github/config/extensions/sqlsmith.cmake`). The harness uses an already installed `sqlsmith` extension and falls back to AST-level
   reduction when it is unavailable; it does not install extensions at runtime.
