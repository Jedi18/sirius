...
/// docs/super-sirius/group-by-bypass.md section 2).
  ...
  const partition_sizing_input& in, int natural)
{
  ...
  // No forced-count override exists on this baseline; see docs/super-sirius/group-by-bypass.md
  // section 1 step 2. Left absent
  // so the precedence is defined if the investigation control is ever merged.
  candidate.forced_num_partitions = std::nullopt;
  ...
  auto const decision = group_by_bypass::decide(candidate);
  {
    ...
    _bypass_decision = decision;
  }
  // Only a bypass this policy actually selected gets a reservation floor. `already_one` is the
  // pre-existing automatic choice and must not acquire new reservation behaviour, and
  // `forced_override` is a benchmark control that was never memory-admitted.
  ...
  // One record per group-by operator, outside any per-row or per-batch loop.
  SIRIUS_LOG_INFO("group_by_bypass: operator_id={} device={} batches={} input_bytes={} {}",
                  this->get_operator_id(),
                  meta.target_device_id,
                  meta.num_batches,
                  meta.total_bytes,
                  group_by_bypass::describe(candidate, decision));
...
{
  // 1. The existing automatic choice, arithmetic untouched.
  int const natural = natural_num_partitions(in.total_bytes, _hash_partition_bytes, _num_gpus);
  // 2-7. The prototype may turn AUTO > 1 into 1. It returns `natural` unchanged in every other
