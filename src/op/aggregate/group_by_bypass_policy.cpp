...
const char* reason_name(decision_reason reason) noexcept
{
  switch (reason) {
    ...
    case decision_reason::forced_override: return "forced_override";
...
decision decide(const candidate_input& in) noexcept
{
  ...
  // 2. An explicit benchmark override wins outright and is never labelled memory-admitted.
  if (in.forced_num_partitions.has_value() && *in.forced_num_partitions != 0) {
    d.num_partitions = *in.forced_num_partitions;
    d.reason         = decision_reason::forced_override;
    return d;
  }

  // 4. Multi-GPU keeps existing behaviour, including the partition floor and placement.
  if (in.num_admitted_gpus > 1) {
    d.reason = decision_reason::multi_gpu;
    return d;
  }

  // 5. AUTO already chose 1. Preserve it, but claim nothing.
  ...
  // 6. AUTO > 1: run the eligibility gates, cheapest and most-common first.
...
std::string describe(const candidate_input& in, const decision& d)
{
  auto opt = [](const std::optional<std::uint64_t>& v) {
    return v.has_value() ? std::to_string(*v) : std::string{"unknown"};
  };
  auto opt_sz = [](const std::optional<std::size_t>& v) {
    return v.has_value() ? std::to_string(*v) : std::string{"unknown"};
  };

  std::ostringstream os;
  os << "reason=" << reason_name(d.reason) << " auto_p=" << in.auto_num_partitions
     << " chosen_p=" << d.num_partitions << " activated=" << (d.prototype_activated ? 1 : 0)
     << " admitted_gpus=" << in.num_admitted_gpus
     << " provenance=" << (in.upstream_complete ? "upstream-complete" : "projected")
     << " rows=" << opt(in.total_rows) << " key_width=" << opt(in.key_width_bytes)
     << " agg_width=" << opt(in.agg_width_bytes)
     << " nullable_keys=" << opt_sz(in.nullable_key_columns)
     << " nullable_aggs=" << opt_sz(in.nullable_agg_columns)
     << " state_ok=" << (in.supported_state ? 1 : 0)
     << " downstream_ok=" << (in.supported_downstream ? 1 : 0)
     << " budget=" << opt(in.admissible_additional_budget)
     << " headroom_fraction=" << in.headroom_fraction << " model_version=" << MODEL_VERSION;
  if (d.model_evaluated) {
    os << " concat=" << d.model.concat_bytes << " hash_set=" << d.model.hash_set_bytes
       << " gather_map=" << d.model.gather_map_bytes << " sparse_agg=" << d.model.sparse_agg_bytes
       << " output=" << d.model.output_bytes << " downstream=" << d.model.downstream_bytes
       << " materialization=" << d.model.materialization_bytes
       << " additional_needed=" << d.model.additional_needed
       << " headroom=" << d.model.headroom_bytes << " required=" << d.model.required_bytes;
  } else {
    os << " model=not-evaluated";
  }
  return os.str();
}
