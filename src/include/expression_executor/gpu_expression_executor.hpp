...
class gpu_expression_executor {
 ...
 public:
  ...
  /**
   * @brief Executes the current set of expressions against the given owned input table.
   *
   * Identical to execute(cudf::table_view), except the executor takes ownership of @p input so
   * that trivial pass-through outputs (BOUND_REF column references) can reuse the input columns
   * zero-copy instead of deep-copying them. A referenced column is moved out of @p input into the
   * output; if the same input column is referenced by more than one output, all but the final
   * reference are copied. Columns of @p input that no output references are freed.
   *
   * Pass-through reclaim is deferred until after every other expression has been evaluated, so a
   * column that is both passed through and read by another expression (e.g. SELECT a, a + 1) is
   * only moved once the expression reading it has finished with the input view.
   *
   * @param input The owned input table to evaluate expressions against (consumed).
   * @return A new table containing the results of expression evaluation.
   */
  std::unique_ptr<cudf::table> execute(std::unique_ptr<cudf::table> input);
 ...
 private:
  ...
  cudf::table_view _input_table;  ///< The input table for expression evaluation (a view, which in
                                  ///< the owning-input path is a view into _owned_input)
  std::unique_ptr<cudf::table>
    _owned_input;  ///< Non-null only on the owning execute() path; lets pass-through outputs reuse
                   ///< (move out) the input columns instead of copying them
  ...
  // Shared implementation of the two public execute() overloads. @p input is the table_view to
  // evaluate against; @p owned_input is non-null only on the owning path, in which case
  // pass-through (BOUND_REF) outputs reuse its columns zero-copy (see execute(unique_ptr)).
  std::unique_ptr<cudf::table> execute_impl(cudf::table_view input,
