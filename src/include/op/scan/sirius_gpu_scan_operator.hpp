...
class sirius_gpu_scan_operator : public sirius_physical_operator {
 public:
  ...
  /// `estimated_cardinality * (bytes emitted / rows emitted)`, from the per-batch counters
  /// below. Both factors are post-filter, so the estimator uses this unscaled. nullopt until a
  /// batch with rows has been emitted.
 ...
 private:
  ...
  /// total that excludes those rows. A batch yielding no size contributes to neither.
