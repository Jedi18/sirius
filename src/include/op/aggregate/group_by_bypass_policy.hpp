...
/// Bumped whenever a term in @ref model_additional_bytes changes, so a decision record from an
/// older build is never compared against a newer model's numbers.
inline constexpr int MODEL_VERSION = 1;
...
enum class decision_reason : std::uint8_t {
  ...
  forced_override,         ///< An explicit benchmark override supplied the count.
...
struct candidate_input {
  ...
  /// An explicit benchmark override, if one exists on this build. Absent on the mainline; see
  /// docs/super-sirius/group-by-bypass.md §1 step 2.
  std::optional<int> forced_num_partitions;
  ...
  /// outstanding reservations. See docs/super-sirius/group-by-bypass.md §3 — this value is already
...
struct decision {
  ...
  memory_model model{};
  /// True only when this prototype changed AUTO > 1 into 1. `already_one` and `forced_override`
  /// are not activations and must never be reported as memory-admitted.
...
/// Apply the decision order from docs/super-sirius/group-by-bypass.md §1. Pure: no locks, no
/// allocation, no logging.
[[nodiscard]] decision decide(const candidate_input& in) noexcept;

/// One-line, greppable decision record body. The caller prefixes query/operator identity.
[[nodiscard]] std::string describe(const candidate_input& in, const decision& d);
