...
class sirius_physical_partition : public sirius_physical_operator {
 public:
  ...
  /// Whether this partition may size itself from a projected total rather than from its whole
  /// input (operator_params::enable_runtime_size_estimation, threaded in at plan generation).
  ///
  /// Read by sirius_pipeline_converter::resolve_barrier, which relaxes the ingress edge to
  /// PARTIAL only for a partition that can actually consult the estimator. Every other
  /// partition keeps the FULL barrier it had before this feature existed, so "feature off" is
  /// the original code path rather than an emulation of it.
 ...
 private:
  ...
  /// Bytes this partition should size itself for: the projected total for everything that will
  /// ever arrive on the input port, floored at what has already arrived (so an undershooting
  /// projection can never ask for fewer partitions than the observed bytes justify) and scaled
  /// by the configured safety factor.
  ///
  /// Returns nullopt when runtime size estimation is disabled or the estimator cannot yet
  /// project a total; callers then fall back to waiting for the whole input. Caches the result
  /// in @ref _size_estimate so the hint and the sizing decision cannot disagree.
  ///
  /// @pre `lock` is held.
