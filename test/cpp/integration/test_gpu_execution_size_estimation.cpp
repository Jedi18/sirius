...
// Runtime coverage for `enable_runtime_size_estimation`, which lets a grouped
// aggregation's PARTITION fix its partition count from a *projected* input total
// instead of waiting for its whole input to arrive.
//
// test_data_size_estimator.cpp covers the estimator's arithmetic against a synthetic
// pipeline DAG. What it cannot cover is the wiring: real scans feeding real
// pipeline_memory_history, a real PARTITION consuming the projection, and a real
// partition count coming out the other side. That is what this file exercises.
//
// The correctness stake is specific. A partition count is frozen the moment it is
// chosen — `murmur3(key) % N` decides which bucket a key lands in, so a later N would
// split one group across two buckets and the merge would emit that group twice. A
// projection that produces a different N than the measured path therefore has to
// produce the same *rows*; every GROUP BY here is compared against DuckDB CPU.
