...
TEST_CASE("group-by bypass gives a forced override precedence without a memory claim",
          "[group_by_bypass][policy]")
{
  auto in                  = good_candidate();
  in.forced_num_partitions = 4;
  auto const d             = decide(in);
  CHECK(d.reason == decision_reason::forced_override);
  CHECK(d.num_partitions == 4);
  CHECK_FALSE(d.prototype_activated);
  CHECK_FALSE(d.model_evaluated);

  SECTION("a forced count of zero is treated as absent")
  {
    auto zero                  = good_candidate();
    zero.forced_num_partitions = 0;
    CHECK(decide(zero).reason == decision_reason::bypass_selected);
  }

  SECTION("an override that forces 1 is still not a memory-admitted bypass")
  {
    auto one                  = good_candidate();
    one.forced_num_partitions = 1;
    auto const forced         = decide(one);
    CHECK(forced.num_partitions == 1);
    CHECK(forced.reason == decision_reason::forced_override);
    CHECK_FALSE(forced.prototype_activated);
  }
}
...
TEST_CASE("group-by bypass decision record names every reason", "[group_by_bypass][policy]")
{
  // A log line that printed "unknown" for a live reason would make rejections undiagnosable.
  decision_reason const all[] = {decision_reason::disabled,
                                 decision_reason::forced_override,
                                 decision_reason::multi_gpu,
                                 decision_reason::already_one,
                                 decision_reason::projected_input,
                                 decision_reason::unknown_metadata,
                                 decision_reason::unsupported_residency,
                                 decision_reason::unsupported_state,
                                 decision_reason::unsupported_downstream,
                                 decision_reason::size_overflow,
                                 decision_reason::insufficient_budget,
                                 decision_reason::bypass_selected};
  for (auto reason : all) {
    INFO("reason value: " << static_cast<int>(reason));
    CHECK(std::string{reason_name(reason)} != "unknown");
  }

  auto const in     = good_candidate();
  auto const d      = decide(in);
  auto const record = describe(in, d);
  CHECK(record.find("reason=bypass_selected") != std::string::npos);
  CHECK(record.find("chosen_p=1") != std::string::npos);
  CHECK(record.find("auto_p=8") != std::string::npos);
  CHECK(record.find("provenance=upstream-complete") != std::string::npos);
  CHECK(record.find("additional_needed=") != std::string::npos);

  SECTION("unknown metadata prints as unknown, never as zero")
  {
    auto unknown       = good_candidate();
    unknown.total_rows = std::nullopt;
    auto const record2 = describe(unknown, decide(unknown));
    CHECK(record2.find("rows=unknown") != std::string::npos);
    CHECK(record2.find("model=not-evaluated") != std::string::npos);
  }
}
